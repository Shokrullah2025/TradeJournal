...
const TrialActivation = ({
  ...
  // intro → card (collect card). On success we go straight to the dashboard —
  // no interstitial screen. `error` shows a recoverable message.
  ...
  const activateTrial = async (pmId) => {
    ...
    try {
      ...
      // Straight into the app — users don't want a "welcome" interstitial after
      // adding their card. Full reload so RequireSubscription re-reads the fresh
      // "trialing" entitlement and the TrialGate clears.
      goWithReload("/dashboard");
    } catch (err) {
