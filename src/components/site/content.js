...
// Slugs (`id`) are the Stripe/subscription_plans keys and must never change —
// `premium` is the plan displayed as "Pro", `enterprise` the one displayed as
// "Elite". Prices here are only the pre-hydration fallback; they must match the
// seeded subscription_plans rows, which are what Stripe actually charges.
