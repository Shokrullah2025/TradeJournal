...
const validatePaymentData = (data) => {
  ...
  if (
    !data.billingCycle ||
    !["monthly", "yearly"].includes(data.billingCycle)
  ) {
...
export const BillingProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const stats = {
      totalSubscribers: mockPayments.filter((p) => p.status === "completed")
        .length,
      ...
      subscriptionsByPlan: {
        basic: 0,
        premium: mockPayments.filter(
          (p) => p.plan === "premium" && p.status === "completed"
        ).length,
        enterprise: mockPayments.filter(
          (p) => p.plan === "enterprise" && p.status === "completed"
        ).length,
      },
        ...
        if (Math.random() > 0.1) {
          const newPayment = {
            ...
            nextBillingDate: new Date(
              Date.now() +
                (paymentData.billingCycle === "monthly" ? 30 : 365) *
                  24 *
                  60 *
                  60 *
                  1000
          ...
          setSubscriptions((prev) => ({
            ...
            subscriptionsByPlan: {
              ...
              [paymentData.plan]:
                prev.subscriptionsByPlan[paymentData.plan] + 1,
            },
          }));
  ...
  const getSubscriptionAnalytics = () => {
    ...
    return {
      ...
      averageRevenuePerUser:
        subscriptions.totalSubscribers > 0
